...
fn the_catalog_covers_every_cli_capability() {
	...
	// Static analysis is independently toggleable, so these track the feature
	// rather than being asserted unconditionally.
	for expected in ["check", "generate"] {
		assert_eq!(
			names.contains(expected),
			cfg!(feature = "analyze"),
			"{expected} should be present exactly when the analyze feature is on"
		);
	}
}
