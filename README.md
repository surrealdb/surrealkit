...
**16 tools**, one per capability: `project_info`, `setup`, `sync`, `seed`, `apply`,
`typegen`, `test`, `init`, and the rollout family (`rollout_baseline`,
`rollout_plan`, `rollout_lint`, `rollout_start`, `rollout_complete`,
`rollout_rollback`, `rollout_repair`, `rollout_status`).
...
`sync --watch` is not exposed: a watch loop never returns, so it cannot be a tool
call. Run it in a terminal. `init --from <git-url>` is CLI-only, because fetching
